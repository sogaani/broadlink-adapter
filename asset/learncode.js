...
class LearnManager extends EventEmitter {
  ...
  startLearning(mac, disableTimeout) {
    ...
    if (!this._device) {
      ...
      setTimeout(function() {
        this.startLearning(mac, disableTimeout);
      }.bind(this), 1000);
      return;
    }
...
async function makeThermostatConfig() {
  ...
  async function learnIR(mode) {
    ...
    const answers = await inquirer.prompt(thermostatQuestions);
    ...
    for (let i = answers.min; i <= answers.max; i++) {
      ...
      const answers = await inquirer.prompt(learnIRCodeQuestions);
...
async function makeOnOffLightConfig() {
  ...
  deviceConfig.ir = {};
  const answers = await inquirer.prompt(onOffLightQuestions);
...
async function makeOnOffSwitchConfig() {
  ...
  deviceConfig.ir = {};
  const answers = await inquirer.prompt(onOffSwitchQuestions);
...
async function makeDimbleLightConfig() {
  ...
  deviceConfig.ir = {};
  const answers = await inquirer.prompt(dimbleLightQuestions);
